...
def main():
    ...
    if args.fit_rank:
        ...
        factors = [np.asarray(f) for f in cp.factors]
        names = ["composition", "charge", "spin", "expert"]
        weights = np.asarray(cp.weights)

        # Which components are just reproducing the mean? A nuisance component has
        # near-FLAT loadings in the charge and spin modes -- it does not
        # differentiate the conditions, it just reconstructs the background level.
        print("\n  per-component loading spread (CV = std/mean, low = flat = mean-like):")
        for r in range(args.fit_rank):
            cvs = {}
            for f, nm in zip(factors, names):
                v = f[:, r]
                cvs[nm] = float(v.std() / (v.mean() + 1e-12))
            flat = cvs["charge"] < 0.15 and cvs["spin"] < 0.15
            tag = "  <-- MEAN-LIKE, likely nuisance" if flat else ""
            print(f"    comp {r} (w={weights[r]:.3g}): " +
                  "  ".join(f"{nm} {cvs[nm]:.2f}" for nm in names) + tag)

        # charge/spin profiles are small enough to just print
        print("\n  charge-mode loadings (rows = charge, cols = component):")
        for i, c in enumerate(args.charges):
            print(f"    q={c:+d}: " + "  ".join(f"{factors[1][i, r]:.3f}"
                                                for r in range(args.fit_rank)))
        print("  spin-mode loadings (rows = spin level k):")
        for k in range(args.spin_levels):
            print(f"    k={k}: " + "  ".join(f"{factors[2][k, r]:.3f}"
                                             for r in range(args.fit_rank)))
        print("  top experts per component:")
        for r in range(args.fit_rank):
            top = np.argsort(factors[3][:, r])[::-1][:5]
            print(f"    comp {r}: " + ", ".join(f"e{int(k)}({factors[3][k, r]:.2f})"
                                                for k in top))

        np.savez(args.out.replace(".npz", "_cp.npz"),
                 weights=weights, **{f"factor_{nm}": f for nm, f in zip(names, factors)})
        print(f"\n  wrote {args.out.replace('.npz', '_cp.npz')}")
        print("  NOTE: rank chosen by hand. Before trusting it, run CORCONDIA and")
        print("        split-half, and refit at neighbouring ranks to check stability.")
